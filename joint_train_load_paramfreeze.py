...
def main(args):
    ...
    # Load generator from a previous checkpoint
    checkpoint_path = 'parent_checkpoints/best_gmodel.pt'
    ...
    # Parameter freezing
    model_dict = generator.state_dict() # Child language spec
    model = torch.load(checkpoint_path) # Parent language model
    pretrained_dict = model.state_dict()    # Parent language model
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict} # Filter unnecessary keys
    new_parent_dict = {}
    # Re-sizing parent model parameters    
    for k, v in pretrained_dict.items():
        parent_size = list(pretrained_dict[k].size())
        child_size = list(model_dict[k].size())
        #print(k, ": ", parent_size, child_size)
        if parent_size[0] > child_size[0]:
            c = v[:child_size[0]]    # Reduce size of tensor to match child model
        elif parent_size[0] < child_size[0]:
            c = model_dict[k]
            c[:parent_size[0]] = v  # Retain all values of parent tensor
        else:
            c = pretrained_dict[k]
        new_parent_dict.update({k:c})    # Update values of parent dict              
    
    model_dict.update(new_parent_dict)  # Update child model with parent model
    generator.load_state_dict(model_dict)
    print("Generator updated from checkpoint")     
    ...
    # Freeze generator parameters (per Zoph et al.)
    print("Freezing generator target word embeddings")
    generator.decoder.embed_tokens.weight.requires_grad = False
